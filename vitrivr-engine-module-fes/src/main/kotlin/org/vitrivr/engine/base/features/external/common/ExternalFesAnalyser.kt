...
abstract class ExternalFesAnalyser<T : ContentElement<*>, U : Descriptor>: Analyser<T, U> {
    ...
    fun analyse(content: T, parameters: Map<String,String>): U {
        return analyse(listOf(content), parameters).first().first()
    }

    fun analyse(content: List<T>, parameters: Map<String, String>): List<List<U>> {
    ...
    abstract fun analyse(content: List<T>, apiWrapper: ApiWrapper, parameters: Map<String, String>): List<List<U>>
