...
abstract class FesExtractor<D:Descriptor, A:ExternalFesAnalyser<ContentElement<*>,D>>(
    ...
    override fun extract(retrievables: List<Retrievable>): List<List<D>> {
        ...
        val resultsByOriginalIndices = batchedResults.mapIndexed { index, batch ->
            val retrievableId = retrievables[indices[index]].id
            IndexedValue(indices[index], batch.map { assignRetrievableId(it, retrievableId) })
        }.groupBy { it.index }
                .mapValues { (_, value) -> value.map { it.value }.flatten() } // Remove the IndexedValue wrapper
