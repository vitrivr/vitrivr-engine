...
class ASR : ExternalFesAnalyser<AudioContent, StringDescriptor>() {
    ...
    override fun analyse(retrievables: Retrievable, api: ApiWrapper, field: Schema.Field<AudioContent, StringDescriptor>?, parameters: Map<String, String>): List<StringDescriptor> {
        val content = retrievables.findContent { it is AudioContent } as List<AudioContent>
        if (content.isEmpty()) return emptyList()
        val result = api.automatedSpeechRecognition(content)
        return result.map { StringDescriptor(UUID.randomUUID(), retrievables.id, StringValue(it), field) }
    }
}