...
companion object {
    ...
    override fun analyse(content: List<ContentElement<*>>, apiWrapper: ApiWrapper, parameters: Map<String, String>): List<FloatVectorDescriptor> {
        ...
        if (imageContents.isEmpty() && !textContents.isEmpty()) {
            ...
            return result.map { FloatVectorDescriptor(UUID.randomUUID(), UUID.randomUUID(), it.map{Value.Float(it)}, true) }
        }
        if (!imageContents.isEmpty() && textContents.isEmpty()) {
    ...
    override fun newExtractor(field: Schema.Field<ContentElement<*>, FloatVectorDescriptor>, input: Operator<Retrievable>, context: IndexContext, persisting: Boolean, parameters: Map<String, Any>): Extractor<ContentElement<*>, FloatVectorDescriptor> {
        require(field.analyser == this) { "The field '${field.fieldName}' analyser does not correspond with this analyser. This is a programmer's error!" }
        return FesExtractor<FloatVectorDescriptor, DenseEmbedding>(input, field, persisting)
    }
