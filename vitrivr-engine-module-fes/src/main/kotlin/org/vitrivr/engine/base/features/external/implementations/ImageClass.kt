...
class ImageClass : ExternalFesAnalyser<ContentElement<*>, LabelDescriptor>() {
    ...
    override fun analyse(content: List<ContentElement<*>>, apiWrapper: ApiWrapper, parameters: Map<String, String>): List<LabelDescriptor> {
        ...
        val descriptors = mutableListOf<LabelDescriptor>()
        for (result in results) {
            // zip the classes with the results
            for ((classIndex, classResult) in result.withIndex()) {
                descriptors.add(LabelDescriptor(UUID.randomUUID(), UUID.randomUUID(), Value.String(classes[classIndex]), Value.Float(classResult), true))
            }
        }
        return descriptors
    }
    ...
    override fun newExtractor(field: Schema.Field<ContentElement<*>, LabelDescriptor>, input: Operator<Retrievable>, context: IndexContext, persisting: Boolean, parameters: Map<String, Any>): Extractor<ContentElement<*>, LabelDescriptor> {
        ...
        return FesExtractor<LabelDescriptor, ImageClass>(input, field, persisting)
    }
}