...
/**
 * Implementation of the [Msb] (Master Shot Boundary) [ExternalAnalyser], which derives the Msv feature from an [VidoContent] as [StructDescriptor].
 *
 * @author Rahel Arnold
 * @version 1.3.0
 */
class Msb : ExternalAnalyser<TextContent, TemporalMetadataDescriptor>() {
    ...
    companion object {
         ...
         */
        fun analyse(content: ContentElement<*>, hostname: String): TemporalMetadataDescriptor {
            ...
            return httpRequest<TemporalMetadataDescriptor>(url, "data=$requestBody")
     ...
     */
    override fun prototype(field: Schema.Field<*, *>) = TemporalMetadataDescriptor(UUID.randomUUID(), UUID.randomUUID(), mapOf())
     ...
     */
    override fun newExtractor(field: Schema.Field<TextContent, TemporalMetadataDescriptor>, input: Operator<Retrievable>, context: IndexContext): MsbExtractor {
        val host: String = field.parameters[HOST_PARAMETER_NAME] ?: HOST_PARAMETER_DEFAULT
        return MsbExtractor(input, this, context[field.fieldName, CONTENT_AUTHORS_KEY]?.split(",")?.toSet(), field, host)
    }
     ...
     */
    override fun newRetrieverForQuery(field: Schema.Field<ContentElement<*>, FloatVectorDescriptor>, query: Query, context: QueryContext): DenseRetriever<TextContent> {
