...
class TSEmbedding : TorchServe<ContentElement<*>, FloatVectorDescriptor>() {
    ...
    override fun newRetrieverForContent(
        ...
        val vectors = this.analyse(content.values, model, host, port, token)
    ...
    override fun newExtractor(
        ...
        context: Context
    ): TorchServeExtractor<ContentElement<*>, FloatVectorDescriptor> {
        val host = context.local[field.fieldName]?.get(TORCHSERVE_HOST_KEY) ?: field.parameters[TORCHSERVE_HOST_KEY]
        ?: "127.0.0.1"
        val port = ((context.local[field.fieldName]?.get(TORCHSERVE_PORT_KEY)
        ...
        val model = context.local[field.fieldName]?.get(TORCHSERVE_MODEL_KEY) ?: field.parameters[TORCHSERVE_MODEL_KEY]
        ?: throw IllegalArgumentException("Missing model for TorchServe model.")
    ...
    override fun toByteString(content: ContentElement<*>): Map<String, ByteString> = when (content) {
        ...
        is TextContent -> mapOf("data" to ByteString.copyFrom(content.text.toByteArray(Charsets.UTF_8)))
