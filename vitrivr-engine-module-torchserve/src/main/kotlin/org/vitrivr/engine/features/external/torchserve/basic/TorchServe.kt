...
abstract class TorchServe<C : ContentElement<*>, D : Descriptor<*>> : Analyser<C, D> {
     ...
     */
    override fun newExtractor(field: Schema.Field<C, D>, input: Operator<Retrievable>, parameters: Map<String, String>, context: IndexContext): TorchServeExtractor<C, D> {
        val host = parameters[TORCHSERVE_HOST_KEY] ?: field.parameters[TORCHSERVE_HOST_KEY] ?: TORCHSERVE_HOST_DEFAULT
        val port = ((parameters[TORCHSERVE_PORT_KEY] ?: field.parameters[TORCHSERVE_PORT_KEY]))?.toIntOrNull() ?: TORCHSERVE_PORT_DEFAULT
        val token = parameters[TORCHSERVE_TOKEN_KEY] ?: field.parameters[TORCHSERVE_TOKEN_KEY]
        val model = parameters[TORCHSERVE_MODEL_KEY] ?: field.parameters[TORCHSERVE_MODEL_KEY] ?: throw IllegalArgumentException("Missing model for TorchServe model.")
     ...
     */
    override fun newExtractor(name: String, input: Operator<Retrievable>, parameters: Map<String, String>, context: IndexContext): TorchServeExtractor<C, D> {
        ...
        val model = parameters[TORCHSERVE_MODEL_KEY] ?: throw IllegalArgumentException("Missing model for TorchServe model.")
