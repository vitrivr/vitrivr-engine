...
import org.vitrivr.engine.module.features.feature.lsc.coordinates.Coordinates
import kotlin.let
import kotlin.time.Duration


/**
 * An integration-style test for the [Coordinates] feature.
 *
...
class CoordinatesTest {
    ...
    fun test() = runTest(timeout = Duration.INFINITE) {
        ...
        val coordinates = Coordinates().let {it.newExtractor(schema.Field("coordinates", it), input = decoder, context = context) }
        val file =  FileSourceMetadata().let { it.newExtractor(schema.Field("file", it), input = coordinates, context = context) }
        ...
        for (r in results) {
            ...
            when (fileDescriptor.path.value.substringAfterLast("/")) {
                "image_with_exif_gps.jpeg" -> {
                    ...
                    val lat = coordinateDescriptor?.values()?.get("lat") as? Value.Double
                    val lon = coordinateDescriptor?.values()?.get("lon") as? Value.Double
                    println("EXIF coords: lat=${lat?.value}, lon=${lon?.value}")
                    assertNotNull(lat, "Latitude value should not be null for EXIF GPS.")
                    assertNotNull(lon, "Longitude value should not be null for EXIF GPS.")
                    assertEquals(47.5598528, lat!!.value, 0.001, "Latitude from EXIF should be correct.")
                    assertEquals(7.5885611, lon!!.value, 0.001, "Longitude from EXIF should be correct.")
                }
                "image_with_comment_gps.jpg" -> {
                    ...
                    val lat = coordinateDescriptor?.values()?.get("lat") as? Value.Double
                    val lon = coordinateDescriptor?.values()?.get("lon") as? Value.Double
                    println("Comment coords: lat=${lat?.value}, lon=${lon?.value}")
                    assertNotNull(lat, "Latitude value should not be null for comment GPS.")
                    assertNotNull(lon, "Longitude value should not be null for comment GPS.")
                    assertEquals(53.3899827, lat!!.value, 0.001, "Latitude from comment should be correct.")
                    assertEquals(-6.1457893, lon!!.value, 0.001, "Longitude from comment should be correct.")
                }
                "image_no_gps.jpg" -> {
                    noGpsFound = true
