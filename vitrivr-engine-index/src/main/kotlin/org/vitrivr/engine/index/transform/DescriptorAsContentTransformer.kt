 ...
 * @version 1.2.0
 */
class DescriptorAsContentTransformer : OperatorFactory {
    /**
    ...
    override fun newOperator(name: String, inputs: Map<String, Operator<out Retrievable>>, context: Context): Operator<out Retrievable> {
        ...
        val fieldName = context[name, "field"]  ?: throw IllegalArgumentException("The descriptor as content transformer requires a field name.")
        return Instance(name, inputs.values.first(), context.contentFactory, fieldName)
    }

    private class Instance(override val name: String, override val input: Operator<out Retrievable>, val contentFactory: ContentFactory, val fieldName: String) : Transformer {
        ...
        private fun convertDescriptor(descriptor: Descriptor<*>): List<TextContent> {
            return when (descriptor) {
                ...
                is StructDescriptor<*> -> descriptor.values().map{ (_,v) -> contentFactory.newTextContent(v.toString()) }
                else -> emptyList()
            }
        }
    }
