...
class WaveExporter: ExporterFactory {
    ...
    override fun newExporter(name: String, input: Operator<Retrievable>, context: IndexContext): Exporter {
        return Instance(input, context, context[name, "contentSources"]?.split(",")?.toSet() )
    }

    /**
     * The [Exporter] generated by this [WaveExporter].
     */
    private class Instance(override val input: Operator<Retrievable>, private val context: IndexContext, private val contentSources:Set<String>?) : Exporter {
        ...
        override fun toFlow(scope: CoroutineScope): Flow<Retrievable> = this.input.toFlow(scope).onEach { retrievable ->
            try {
                ...
                val content = retrievable.content.filterIsInstance<AudioContent>().filter { contentIds?.contains(it.id) ?: true }
