...
class QueryParser(val schema: Schema) {
     ...
     */
    private fun parseRetrieverOperator(
        description: InformationNeedDescription,
        operatorName: String,
        content: MutableMap<String, ContentElement<*>>
    ): Operator<out Retrievable> {
        /* Extract necessary information. */
        val operation = description.operations[operatorName] as? RetrieverDescription ?: throw IllegalArgumentException(
            "Operation '$operatorName' not found in information need description."
        )
        val input = description.inputs[operation.input]
            ?: throw IllegalArgumentException("Input '${operation.input}' for operation '$operatorName' not found")
        ...
        if (operation.field == null) { //special case, handle pass-through
            ...
            return RetrievedLookup(
                this.schema.connection.getRetrievableReader(),
                listOf(UUID.fromString((input as RetrievableIdInputData).id)),
                "${operatorName}-lookup"
            )
        }
        ...
        val field = this.schema[fieldAndAttributeName.first]
            ?: throw IllegalArgumentException("Retriever '${operation.field}' not defined in schema")
        ...
        return when (input) {
            ...
            is VectorInputData -> field.getRetrieverForDescriptor(
                FloatVectorDescriptor(vector = Value.FloatVector(input.data.toFloatArray())),
                description.context
            )
                ...
                if (fieldAndAttributeName.second != null && input.comparison != null) {
                    /* yes */
                    val subfield =
                        field.analyser.prototype(field).layout().find { it.name == fieldAndAttributeName.second }
                    ...
                    field.getRetrieverForQuery(
                        SimpleBooleanQuery(
                            value,
                            ComparisonOperator.fromString(input.comparison!!),
                            fieldAndAttributeName.second,
                            limit
                        ),
                        description.context
                    )
                    ...
                    /* no */
                    field.getRetrieverForContent(
                        content.computeIfAbsent(operation.input) { input.toContent() },
                        description.context
                    )
                }
            }
     ...
     * @return [Operator] instance.
     */
    private fun parseTransformationOperator(
        description: InformationNeedDescription,
        operatorName: String,
        operators: Map<String, Operator<out Retrievable>>
    ): Operator<Retrievable> {
        val operation = description.operations[operatorName] as? TransformerDescription
            ?: throw IllegalArgumentException("Operation '$operatorName' not found in information need description.")
        val input = operators[operation.input]
            ?: throw IllegalArgumentException("Input '${operation.input}' for operation '$operatorName' not found")
        ...
        return factory.newTransformer(operatorName, input, emptyMap(), description.context)
    }
     ...
     * @return [Operator] instance.
     */
    private fun parseAggregationOperator(
        description: InformationNeedDescription,
        operatorName: String,
        operators: Map<String, Operator<out Retrievable>>
    ): Operator<Retrievable> {
        val operation = description.operations[operatorName] as? AggregatorDescription
            ?: throw IllegalArgumentException("Operation '$operatorName' not found in information need description.")
        ...
        val factory = loadServiceForName<AggregatorFactory>(operation.aggregatorName + "Factory")
            ?: throw IllegalArgumentException("No factory found for '${operation.aggregatorName}'")
