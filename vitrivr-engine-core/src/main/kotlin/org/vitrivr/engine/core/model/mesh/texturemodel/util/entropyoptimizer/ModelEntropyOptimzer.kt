...
object ModelEntropyOptimizer {
    ...
    private fun calculateEntropyRelativeToTotalArea_old(normals: List<Vector3f>, viewVector: Vector3f): Float {
        val areas = ArrayList<Float>(normals.size)
        val projected = ArrayList<Float>(normals.size)
        ...
        val logRelativeProjected = relativeProjected.stream().map { obj: Float? -> log2(obj?:0f) }
            .toList()
