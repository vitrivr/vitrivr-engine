...
/**
 * A [Decoder] that can decode [PointCloudDecoder] from a [Source] of [MediaType.POINTCLOUD].
 *
 * @author Rahel Arnold
 * @version 1.1.0
 */
class PointCloudDecoder : DecoderFactory {
    /**
    ...
    /**
     * The [Decoder] returned by this [PointCloudDecoder].
     */
    private class Instance(override val input: Enumerator, private val context: IndexContext) : Decoder {
        ...
        /**
         * Converts this [PointCloudDecoder] to a [Flow] of [Retrievable] elements.
         *
        ...
        override fun toFlow(scope: CoroutineScope): Flow<Retrievable> = this.input.toFlow(scope).mapNotNull { sourceRetrievable ->
            ...
            if (source.type != MediaType.POINTCLOUD) {
