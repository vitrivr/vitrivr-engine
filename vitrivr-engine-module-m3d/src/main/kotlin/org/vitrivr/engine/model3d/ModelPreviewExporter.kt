 ...
 * @version 2.2.0
 */
class ModelPreviewExporter : OperatorFactory {
    companion object {
    ...
    override fun newOperator(name: String, inputs: Map<String, Operator<out Retrievable>>, context: Context): Operator<out Retrievable> {
        require(inputs.size == 1) { "ModelPreviewExporter requires a single input. Make merge explicit" }
        val input = inputs.values.first() as Operator<Retrievable>
