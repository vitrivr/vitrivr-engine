...
public final class ModelLoader {
    ...
    public static Model loadModel(String modelId, String modelPath) {
        var model = loadModel(modelId, modelPath, aiProcess_JoinIdenticalVertices | aiProcess_GlobalScale | aiProcess_FixInfacingNormals | aiProcess_Triangulate | aiProcess_CalcTangentSpace | aiProcess_LimitBoneWeights | aiProcess_PreTransformVertices);
    ...
    public static Model loadModel(String modelId, String modelPath, int flags) {
        ...
        for (var ic = 0; ic < numMaterials; ic++) {
            ...
            materialList.add(ModelLoader.processMaterial(aiMaterial, modelDir, aiScene));
        }
     ...
     */
    private static Material processMaterial(AIMaterial aiMaterial, String modelDir, AIScene aiScene) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (texturePath != null && texturePath.length() > 0) {
                if (texturePath.charAt(0) == '*') {
                    ...
                    var image = loadEmbeddedTexture(aiScene, textureIndex);
                    if (image != null) {
                        material.setTexture(new Texture(image));
     ...
     */
    private static BufferedImage loadEmbeddedTexture(AIScene aiScene, int textureIndex) {
        try {
            ...
            AITexture aiTexture = AITexture.create(aiTextures.get(textureIndex));
            if (aiTexture == null) {
                LOGGER.error("Failed to retrieve texture at index {}", textureIndex);
                return null;
            }

            long address = aiTexture.pcData().address0();
            ByteBuffer buffer = MemoryUtil.memByteBuffer(address, aiTexture.mWidth());

            // Read the data into a byte array
            byte[] texData = new byte[buffer.remaining()];
            buffer.get(texData);

            // Create BufferedImage from decoded data
            ByteArrayInputStream bis = new ByteArrayInputStream(texData);
